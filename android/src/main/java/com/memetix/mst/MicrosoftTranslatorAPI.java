...
public abstract class MicrosoftTranslatorAPI {
    ...
    private static String subscriptionKey = "80e4e888083549658be9a344ea1146c0";
...
//        wr.flush();
     ...
     */
    protected  String retrieveResponse(final URL url) throws Exception {
        if (subscriptionKey != null && System.currentTimeMillis() > tokenExpiration) {
            ...
            return new getAccessToken().doInBackground();
        }
        return null;
    }
...
//            Log.e("TaGgGGggG",response);
    ...
    protected class getAccessToken extends AsyncTask<URL, Void, String> {
        ...
        protected String doInBackground(URL... urls) {
            try {
                ...
                try {
                    return new getResponseCode().doInBackground(uc);
                } finally {
                    if (uc != null) {
                        uc.disconnect();
                    }
    ...
    protected class getResponseCode extends AsyncTask<HttpURLConnection, Void, String> {

        private Exception exception;

        protected String doInBackground(HttpURLConnection... uc) {
            try {
                int responseCode  = uc[0].getResponseCode();
                final String result = inputStreamToString(uc[0].getInputStream());
                if (responseCode != 200) {
                    throw new Exception("Error from Microsoft Translator API: " + result);
                }
                Log.e("TAG","bjkbjhbj");
